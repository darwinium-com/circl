...
func (p PeerSigner) CheckKeyShare(shareCom SharesCommitment) bool {
	vss := secretsharing.NewFeldmanSecretSharing(uint(p.threshold))
	return vss.Verify(p.keyShare, shareCom)
}
